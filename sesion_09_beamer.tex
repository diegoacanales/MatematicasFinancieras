...
\begin{frame}{Cálculo de la TIR}
    \begin{alertblock}{No hay solución algebraica directa}
        ...
        \begin{itemize}
            ...
            \item Calculadora financiera (HP 17bII+)
...
\begin{frame}{HP 17bII+: Cálculo de VPN}
    \begin{block}{Problema}
    ...
    \textbf{Ruta:} \texttt{FIN} $\to$ \texttt{CFLO}
    \begin{center}
    \scriptsize
    \begin{tabular}{@{}lll@{}}
        \toprule
        ...
        \midrule
        \texttt{FIN} $\to$ \texttt{CFLO} & FLOW(0)=? & Entrar al menú de flujos \\
        \texttt{50000 +/- INPUT} & FLOW(0)=-50,000 & Inversión inicial \\
        \texttt{15000 INPUT} & FLOW(1)=15,000 & Flujo año 1 \\
        \texttt{INPUT} & \#TIMES(1)=1 & Se repite 1 vez \\
        \texttt{20000 INPUT} & FLOW(2)=20,000 & Flujo año 2 \\
        \texttt{INPUT} & \#TIMES(2)=1 & Se repite 1 vez \\
        \texttt{25000 INPUT} & FLOW(3)=25,000 & Flujo año 3 \\
        \texttt{INPUT} & \#TIMES(3)=1 & Se repite 1 vez \\
        \texttt{CALC} & & Ir a cálculos \\
        \texttt{12 I\%} & 12 & Tasa de descuento \\
        \texttt{NPV} & \textbf{1,103.19} & VPN \\
        \bottomrule
    \end{tabular}
    \end{center}
...
\begin{frame}{HP 17bII+: Cálculo de TIR}
    \textbf{Continuando del ejemplo anterior} (dentro de \texttt{CALC})...
    ...
    \begin{tabular}{@{}lll@{}}
        ...
        \textbf{Teclas} & \textbf{Display} & \textbf{Descripción} \\
        \midrule
        \texttt{IRR\%} & \textbf{13.67} & TIR del proyecto \\
        \bottomrule
    \end{tabular}
    \end{center}
...
\begin{frame}{HP 17bII+: Flujos con Repetición}
    \begin{block}{Problema}
    ...
    \begin{tabular}{@{}lll@{}}
        ...
        \textbf{Teclas} & \textbf{Display} & \textbf{Descripción} \\
        \midrule
        \texttt{100000 +/- INPUT} & FLOW(0)=-100,000 & Inversión \\
        \texttt{30000 INPUT} & FLOW(1)=30,000 & Flujo periódico \\
        \texttt{5 INPUT} & \#TIMES(1)=5 & Se repite 5 veces \\
        \texttt{CALC} & & Ir a cálculos \\
        \texttt{10 I\%} & 10 & Tasa \\
        \texttt{NPV} & \textbf{13,723.60} & VPN \\
        \texttt{IRR\%} & \textbf{15.24} & TIR \\
        \bottomrule
    \end{tabular}
    \end{center}

    \texttt{\#TIMES} indica cuántas veces se repite el flujo (default = 1).
\end{frame}
...
\begin{frame}{Conceptos Clave}
    \begin{enumerate}
        ...
        \item HP 17bII+: \texttt{FIN} $\to$ \texttt{CFLO} para flujos, \texttt{CALC} $\to$ \texttt{NPV}/\texttt{IRR\%}
    \end{enumerate}
\end{frame}
...
\begin{frame}{Tarea para la Próxima Sesión}
    \begin{enumerate}
        ...
        \item \textbf{HP 17bII+:} Practica ingresando flujos irregulares en \texttt{FIN} $\to$ \texttt{CFLO} y calculando VPN/TIR.
