...
\begin{frame}{HP 17bII+: Menú de Conversión de Tasas (ICNV)}
    La HP 17bII+ tiene una función \textbf{dedicada} para conversión de tasas.

    \vspace{0.3cm}

    \textbf{Ruta:} \texttt{FIN} $\to$ \texttt{ICNV} (Interest CoNVersion)
    ...
    \begin{tabular}{@{}cl@{}}
        ...
        \midrule
        \texttt{NOM\%} & Tasa nominal anual \\
        \texttt{EFF\%} & Tasa efectiva anual \\
        \texttt{P} & Períodos de capitalización por año ($m$) \\
        \bottomrule
    \end{tabular}
    \end{center}
    ...
    \begin{alertblock}{Procedimiento}
        Ingresa \textbf{dos} valores cualesquiera y la calculadora resuelve el tercero.

        Ejemplo: ingresa \texttt{NOM\%} y \texttt{P}, presiona \texttt{EFF\%} para obtener la tasa efectiva.
    \end{alertblock}
\end{frame}

\begin{frame}{HP 17bII+: Ejemplo - Nominal a Efectiva}
    \begin{block}{Problema}
    ...
    \begin{tabular}{@{}lll@{}}
        ...
        \midrule
        \texttt{FIN} $\to$ \texttt{ICNV} & & Entrar al menú ICNV \\
        \texttt{12 NOM\%} & 12.00 & Tasa nominal 12\% \\
        \texttt{12 P} & 12.00 & 12 períodos (mensual) \\
        \texttt{EFF\%} & \textbf{12.68} & Tasa efectiva anual \\
        \bottomrule
    \end{tabular}
    \end{center}
    ...
    \textbf{Tasa efectiva = 12.68\%}

    \vspace{0.2cm}
    Verificación: $(1 + 0.12/12)^{12} - 1 = (1.01)^{12} - 1 = 12.68\%$ \checkmark
\end{frame}

\begin{frame}{HP 17bII+: Ejemplo - Comparar Ofertas}
    \begin{block}{Problema}
    ...
    \textbf{Banco A} (\texttt{FIN} $\to$ \texttt{ICNV}):
    \begin{center}
    \begin{tabular}{@{}lll@{}}
        \texttt{11.5 NOM\%}, \texttt{4 P}, \texttt{EFF\%} & $\to$ \textbf{11.99\%} & Tasa efectiva \\
    \end{tabular}
    \end{center}
    ...
    \textbf{Banco B} (misma ruta):
    \begin{center}
    \begin{tabular}{@{}lll@{}}
        \texttt{11.3 NOM\%}, \texttt{12 P}, \texttt{EFF\%} & $\to$ \textbf{11.91\%} & Tasa efectiva \\
    \end{tabular}
    \end{center}
